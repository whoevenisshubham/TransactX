...
M1-3B uses the existing `idempotency_records` table without a new migration. `POST /api/payments` canonicalizes the source account, normalized recipient identifier, positive amount, and currency into a deterministic SHA-256 request hash. Payment and idempotency rows are inserted in the same PostgreSQL transaction. The unique `(user_id, key)` constraint arbitrates concurrent requests; exact hash matches return the linked payment, while hash mismatches return `409 Conflict`. No account balance, ledger table, or route is changed in this phase.

### FUTURE WORK

Settlement execution, ledger balancing logic, financial concurrency control, bank adapters, routing, and payment seed data remain future implementation work. `backend/cmd/devseed` is a development-only provisioning command for the synthetic bank and OPS_ADMIN; its password must be supplied through `DEV_ADMIN_PASSWORD` and is never stored in the repository.