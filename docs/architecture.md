...
- **Target Isolation & Validation**: All scenarios are strictly target-scoped by switch-level `executionTargetID` (e.g. `RAIL-A`) and explicit non-bank health target identities via `BuildTargetValidator(explicitHealthTargetIDs, executionTargetIDs, bankCodes...)`. Bank ownership codes (`sourceBankID`, `destinationBankID`, legacy bank health keys like `BANK-A`, `BANK-B`) are strictly excluded to prevent conflating logical bank ownership with switch-level execution rail identities. Nonexistent or unconfigured target IDs are rejected (`ErrTargetNotFound`). Injected faults on target $X$ never impact target $Y$, unrelated banks, or unrelated payment flows.
- **Lifecycle & Exact Expiry**:
  ...
  - `RESET`: Clears active scenarios for a target or globally in both memory and PostgreSQL within a single atomic transaction. Crucially, `RESET` classifies active scenarios deterministically against the current time:
  ...
  - `AUTO-EXPIRY`: Evaluated deterministically against the unified expiry boundary: `expires_at <= now => EXPIRED` (`!now.Before(expiresAt)`). Expired scenarios transition to inactive in PostgreSQL via atomic check-and-update (`active = false, stopped_at = expires_at, stopped_by = 'SYSTEM_AUTO_EXPIRY'`) and emit `CHAOS_EXPIRED` exactly once. If persistence fails during expiry, the error is surfaced and in-memory state is NOT silently deactivated, preserving memory/database consistency.
- **Restart Safety & Hydration**: Active chaos state survives process restarts. On startup, `Hydrate()` queries all active scenarios from PostgreSQL; any discovered active scenario that has reached or passed expiry (`expires_at <= now`) is atomically finalized to inactive (`active=false, stopped_at=expires_at, stopped_by=SYSTEM_AUTO_EXPIRY`) with exactly one `CHAOS_EXPIRED` event, while unexpired scenarios are hydrated into memory cache. Similarly, `Start()` inspects durable storage for active rows on the target and atomically finalizes any stale expired scenarios before starting a new scenario, preventing stale unfinalized rows from persisting in PostgreSQL. If `Hydrate()` fails due to a database error, startup logs an error and terminates with non-zero exit to avoid running with unhydrated chaos state. If an unexpired active scenario exists in the database but is absent from memory cache, `GetActiveFault` loads, validates, and caches it on demand.
