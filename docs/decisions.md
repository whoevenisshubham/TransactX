...
- **Fatal Startup Hydration**: Active chaos state survives process restarts. On startup, `Hydrate()` reloads unexpired active scenarios from PostgreSQL. If hydration fails due to a database or connection error, startup logs a fatal error and terminates (`os.Exit(1)`) rather than running with incomplete, partitioned, or un-hydrated chaos state.
- **Lifecycle & Exactly-Once Expiry Guarantees**:
  ...
  - `AUTO-EXPIRY`: Evaluated against injectable clock (`now.After(expiresAt)`). Expired scenarios automatically become inactive in PostgreSQL and emit `CHAOS_EXPIRED` exactly once using a single atomic transaction with `WHERE scenario_id = $1 AND active = true AND expires_at <= $2 RETURNING scenario_id...`.
