...
import { Amount, Avatar, BrandMark, Button, CopyButton, EmptyState, formatDate, Icon, PageHeader, PaymentRow, paymentResultCopy, Skeleton, StatusBadge } from "./components";
...
// ... (skipping to UncertainPayment & DetailsView updates)
...
function LoginForm({ onAuthenticated }: { onAuthenticated: (token: string) => void }) { const [identifier, setIdentifier] = useState(""); const [password, setPassword] = useState(""); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(""); try { const result = await api.login(identifier, password); onAuthenticated(result.token); } catch (caught) { setError(caught instanceof Error ? caught.message : "We couldn't sign you in."); } finally { setBusy(false); } } return <form className="auth-form" onSubmit={submit}><div className="form-heading"><p className="eyebrow">Welcome back</p><h2>Sign in to continue</h2><p>Access your balance and payment activity.</p></div><Field label="Phone or payment ID" value={identifier} onChange={setIdentifier} placeholder="you@transactx" autoComplete="username" /><Field label="Password" value={password} onChange={setPassword} type="password" autoComplete="current-password" />{error && <InlineError message={error} />}<Button type="submit" disabled={busy}>{busy ? "Signing in…" : "Sign in"}<Icon name="arrow" /></Button></form>; }
function RegisterForm({ onAuthenticated }: { onAuthenticated: (token: string) => void }) { const [form, setForm] = useState({ name: "", phone: "", paymentIdentifier: "", password: "", role: "CUSTOMER" }); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); function update(field: keyof typeof form, value: string) { setForm((current) => ({ ...current, [field]: value })); } async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(""); try { await api.register(form); const result = await api.login(form.paymentIdentifier, form.password); onAuthenticated(result.token); } catch (caught) { setError(caught instanceof Error ? caught.message : "We couldn't create your account."); } finally { setBusy(false); } } return <form className="auth-form auth-form-register" onSubmit={submit}><div className="form-heading"><p className="eyebrow">Start with TransactX</p><h2>Create your account</h2><p>A simple account for clear, deliberate payments.</p></div><div className="form-grid"><Field label="Full name" value={form.name} onChange={(value) => update("name", value)} autoComplete="name" /><Field label="Phone" value={form.phone} onChange={(value) => update("phone", value)} autoComplete="tel" /><Field className="form-grid-wide" label="Payment ID" value={form.paymentIdentifier} onChange={(value) => update("paymentIdentifier", value)} placeholder="you@transactx" autoComplete="username" /><Field className="form-grid-wide" label="Password" value={form.password} onChange={(value) => update("password", value)} type="password" autoComplete="new-password" minLength={8} /></div>{error && <InlineError message={error} />}<Button type="submit" disabled={busy}>{busy ? "Creating account…" : "Create account"}<Icon name="arrow" /></Button></form>; }
...
// ---------------------------------------------------------------------------
// Customer Shell (unchanged from original)
// ---------------------------------------------------------------------------

function CustomerShell({ token, onLogout }: { token: string; onLogout: () => void }) {
  const [user, setUser] = useState<User | null>(null); const [accounts, setAccounts] = useState<Account[]>([]); const [payments, setPayments] = useState<Payment[]>([]); const [queue, setQueue] = useState<OfflineIntentQueue | null>(null); const [queueIntents, setQueueIntents] = useState<OfflineIntent[]>([]); const [online, setOnline] = useState(() => navigator.onLine); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [view, setView] = useState<View>(readView()); const [selectedPaymentID, setSelectedPaymentID] = useState<string | null>(readPaymentID());
  useEffect(() => { let active = true; Promise.all([api.me(token), api.accounts(token), api.payments(token)]).then(([profile, accountList, paymentList]) => { if (active) { setQueue(new OfflineIntentQueue(undefined, profile.id)); setUser(profile); setAccounts(accountList); setPayments(paymentList); setLoading(false); } }).catch((caught) => { if (!active) return; if (caught instanceof ApiError && caught.status === 401) onLogout(); else { setError(caught instanceof Error ? caught.message : "We couldn't load your account."); setLoading(false); } }); return () => { active = false; }; }, [token, onLogout]);
  async function refreshQueue(scopedQueue = queue) { if (!scopedQueue) return; const states: OfflineIntent["state"][] = ["QUEUED", "SYNCING", "SYNCED", "RETRYABLE", "FAILED"]; const records = (await Promise.all(states.map((state) => scopedQueue.listByState(state)))).flat(); setQueueIntents(records.sort((left, right) => right.updatedAt.localeCompare(left.updatedAt))); }
  useEffect(() => { if (!queue) return; let active = true; const worker = new OfflineReplayWorker(queue, { createPayment: async (payload, idempotencyKey, clientRequestId) => { const response = await api.createPaymentWithStatus(payload, token, idempotencyKey, clientRequestId); return { payment: response.data, status: response.status }; }, getPayment: (paymentID) => api.payment(paymentID, token) }); const replay = async () => { await worker.run(); if (active) { await refreshQueue(queue); api.payments(token).then(setPayments).catch(() => undefined); } }; const handleOnline = () => { setOnline(true); void replay(); }; const handleOffline = () => setOnline(false); window.addEventListener("online", handleOnline); window.addEventListener("offline", handleOffline); void replay(); return () => { active = false; worker.stop(); window.removeEventListener("online", handleOnline); window.removeEventListener("offline", handleOffline); void queue.close(); }; }, [queue, token]);
  ...
  async function retryIntent(clientRequestId: string) { if (!queue) return; await queue.retryFailed(clientRequestId); const worker = new OfflineReplayWorker(queue, { createPayment: async (payload, idempotencyKey, requestId) => { const response = await api.createPaymentWithStatus(payload, token, idempotencyKey, requestId); return { payment: response.data, status: response.status }; }, getPayment: (paymentID) => api.payment(paymentID, token) }); await worker.run(); await refreshQueue(queue); refreshPayments(); }
  function queueChanged() { void refreshQueue(); }
  ...
  return <div className="product-shell"><Sidebar view={view} user={user} onNavigate={navigate} onLogout={onLogout} /><main className="main-content"><MobileHeader user={user} onLogout={onLogout} /><div className="content-wrap"><NetworkStatus online={online} />{view === "home" && <HomeView user={user} account={account} payments={payments} onNavigate={navigate} />}{view === "pay" && queue && <PayView account={account} token={token} queue={queue} onComplete={completedPayment} onQueueChanged={queueChanged} onNavigate={navigate} />}{view === "transactions" && <TransactionsView payments={payments} onNavigate={navigate} />}{view === "details" && selectedPaymentID && <DetailsView paymentID={selectedPaymentID} token={token} onNavigate={navigate} />}<OfflineQueuePanel intents={queueIntents} onRetry={retryIntent} /></div></main></div>;
}
...
function HomeView({ user, account, payments, onNavigate }: { user: User | null; account?: Account; payments: Payment[]; onNavigate: (view: View, paymentID?: string) => void }) { return <><PageHeader eyebrow="Overview" title={`${timeGreeting()}, ${firstName(user?.name)}`} description="Your account at a glance." action={<Button onClick={() => onNavigate("pay")}><Icon name="send" />Send money</Button>} /><section className="overview-grid"><div className="balance-panel"><div className="section-kicker">Available balance</div><Amount paise={account?.balancePaise ?? 0} prominent /><div className="balance-foot"><span>{account?.accountNumber ?? "Account unavailable"}</span><span className="account-status"><span className="status-dot" />{account?.status === "ACTIVE" ? "Active" : account?.status ?? "Unavailable"}</span></div></div><div className="account-note"><span className="note-index">01</span><div><strong>Ready when you are</strong><p>Send a payment using a TransactX ID. You will review the recipient, amount, and note before anything is submitted.</p><button className="text-link" onClick={() => onNavigate("pay")}>Start a payment <Icon name="arrow" size={15} /></button></div></div></section><section className="section-block"><div className="section-heading"><div><p className="eyebrow">Activity</p><h2>Recent transactions</h2></div>{payments.length > 0 && <button className="text-link" onClick={() => onNavigate("transactions")}>View all <Icon name="arrow" size={15} /></button>}</div>{payments.length === 0 ? <EmptyState title="No transactions yet" description="Your payments will appear here after your first transfer." action={<Button variant="secondary" onClick={() => onNavigate("pay")}>Send money</Button>} /> : <div className="payment-list">{payments.slice(0, 5).map((payment) => <PaymentRow key={payment.id} payment={payment} onClick={() => onNavigate("details", payment.id)} />)}</div>}</section><section className="account-summary"><div><p className="eyebrow">Account details</p><h2>Your TransactX identity</h2></div><div className="summary-detail"><span>Payment ID</span><strong>{user?.paymentIdentifier}</strong></div><div className="summary-detail"><span>Account number</span><strong>{account?.accountNumber ?? "—"}</strong></div></section></>; }

function PayView({ account, token, queue, onComplete, onQueueChanged, onNavigate }: { account?: Account; token: string; queue: OfflineIntentQueue; onComplete: (payment: Payment) => void; onQueueChanged: () => void; onNavigate: (view: View) => void }) {
  const [stage, setStage] = useState<"form" | "confirm" | "processing" | "result" | "uncertain" | "queued">("form"); const [recipientInput, setRecipientInput] = useState(""); const [recipient, setRecipient] = useState<Recipient | null>(null); const [amount, setAmount] = useState(""); const [note, setNote] = useState(""); const [lookupBusy, setLookupBusy] = useState(false); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [payment, setPayment] = useState<Payment | null>(null); const [idempotencyKey, setIdempotencyKey] = useState<string | null>(null); const [clientRequestId, setClientRequestId] = useState<string | null>(null); const submitLock = useRef(false);
  const amountPaise = parsePaise(amount);
  async function resolve(event?: FormEvent) { event?.preventDefault(); if (!recipientInput.trim()) return; setLookupBusy(true); setError(""); setRecipient(null); setIdempotencyKey(null); setClientRequestId(null); try { setRecipient(await api.resolveRecipient(recipientInput.trim(), token)); } catch (caught) { setError(caught instanceof Error ? caught.message : "Payment ID not found"); } finally { setLookupBusy(false); } }
  function continueToConfirm(event: FormEvent) { event.preventDefault(); if (!recipient) { void resolve(); return; } if (amountPaise === null) { setError("Enter a valid amount with up to two decimal places."); return; } if (amountPaise > (account?.balancePaise ?? 0)) { setError("Your balance is too low for this payment."); return; } if (note.trim().length > 280) { setError("Keep your note under 280 characters."); return; } setError(""); setIdempotencyKey(crypto.randomUUID()); setClientRequestId(crypto.randomUUID()); setStage("confirm"); }
  async function submitPayment() { if (!recipient || amountPaise === null || submitLock.current) return; submitLock.current = true; const key = idempotencyKey ?? crypto.randomUUID(); const requestID = clientRequestId ?? crypto.randomUUID(); const payload = { recipient: recipient.paymentIdentifier, amountPaise, currency: "INR", note: note.trim() || undefined }; setIdempotencyKey(key); setClientRequestId(requestID); setStage("processing"); setBusy(true); setError(""); try { const result = await api.createPayment(payload, token, key, requestID); setPayment(result); setStage("result"); } catch (caught) { if (caught instanceof ApiError && caught.status === 0) { try { await queue.enqueue(payload, { clientRequestId: requestID, idempotencyKey: key }); onQueueChanged(); setStage("queued"); } catch { setError("We couldn't save this payment on your device. Please try again."); setStage("form"); } } else { setError(paymentError(caught)); setIdempotencyKey(null); setClientRequestId(null); setStage("form"); } } finally { submitLock.current = false; setBusy(false); } }
  function editPayment() { setIdempotencyKey(null); setClientRequestId(null); setStage("form"); }
  if (stage === "confirm") return <ConfirmPayment recipient={recipient!} amount={amountPaise!} note={note} onBack={editPayment} onConfirm={submitPayment} busy={busy} />;
  if (stage === "processing") return <ProcessingPayment recipient={recipient!} amount={amountPaise!} />;
  if (stage === "queued") return <QueuedPayment recipient={recipient!} amount={amountPaise!} onDone={() => onNavigate("home")} />;
  if (stage === "uncertain") return <UncertainPayment onRetry={submitPayment} onEdit={editPayment} />;
  if (stage === "result" && payment) return <PaymentResult payment={payment} token={token} onPayment={setPayment} onView={() => onComplete(payment)} onDone={() => onNavigate("home")} />;
  return <><PageHeader eyebrow="New payment" title="Send money" description="Review the recipient, amount, and note before you confirm." /><form className="pay-form" onSubmit={continueToConfirm}><div className="form-section"><div className="form-section-number">01</div><div className="form-section-body"><label className="field field-large"><span>Recipient payment ID</span><div className="input-with-action"><input value={recipientInput} onChange={(event) => { setRecipientInput(event.target.value); setRecipient(null); setError(""); setIdempotencyKey(null); }} onBlur={() => { if (recipientInput && !recipient) void resolve(); }} placeholder="name@transactx" autoComplete="off" /><button type="button" className="input-action" onClick={() => void resolve()} disabled={lookupBusy}>{lookupBusy ? "Checking…" : "Find"}</button></div><small className="field-hint">Use the recipient's TransactX payment ID.</small></label>{recipient && <div className="recipient-confirmed"><Avatar name={recipient.name} /><span><strong>{recipient.name}</strong><small>{recipient.paymentIdentifier}</small></span><Icon name="check" /></div>}</div></div><div className="form-section"><div className="form-section-number">02</div><div className="form-section-body"><label className="field field-large"><span>Amount</span><div className="amount-input"><span>₹</span><input value={amount} onChange={(event) => { setAmount(event.target.value); setError(""); setIdempotencyKey(null); }} inputMode="decimal" placeholder="0.00" aria-label="Amount in Indian rupees" /></div><small className="field-hint">Available to send: <Amount paise={account?.balancePaise ?? 0} /></small></label></div></div><div className="form-section"><div className="form-section-number">03</div><div className="form-section-body"><label className="field field-large"><span>Note <small className="optional-label">Optional</small></span><input value={note} maxLength={280} onChange={(event) => { setNote(event.target.value); setIdempotencyKey(null); }} placeholder="What's this payment for?" /><small className="field-hint">{note.length}/280 · Included in your payment record.</small></label></div></div>{error && <InlineError message={error} />}<div className="pay-actions"><Button type="button" variant="quiet" onClick={() => onNavigate("home")}>Cancel</Button><Button type="submit" disabled={!recipient || amountPaise === null}>Review payment <Icon name="arrow" /></Button></div></form></>;
}

function ConfirmPayment({ recipient, amount, note, onBack, onConfirm, busy }: { recipient: Recipient; amount: number; note: string; onBack: () => void; onConfirm: () => void; busy: boolean }) { return <div className="focused-state"><button className="back-link" onClick={onBack}><Icon name="arrow" size={15} />Back to payment</button><div className="confirm-heading"><p className="eyebrow">Review payment</p><h1>Check the details</h1><p>Make sure everything looks right before you confirm.</p></div><div className="confirm-sheet"><div className="confirm-recipient"><Avatar name={recipient.name} /><span><small>Sending to</small><strong>{recipient.name}</strong><span>{recipient.paymentIdentifier}</span></span></div><div className="confirm-amount"><small>Amount</small><Amount paise={amount} prominent /></div>{note.trim() && <div className="confirm-message"><small>Note</small><strong>{note.trim()}</strong></div>}<div className="confirm-note"><span><Icon name="check" size={15} />Recipient verified</span><span><Icon name="check" size={15} />INR payment</span></div></div><div className="confirm-actions"><Button variant="secondary" onClick={onBack}>Edit payment</Button><Button onClick={onConfirm} disabled={busy}>{busy ? "Confirming…" : "Confirm payment"}<Icon name="arrow" /></Button></div></div>; }
function QueuedPayment({ recipient, amount, onDone }: { recipient: Recipient; amount: number; onDone: () => void }) { return <div className="focused-state centered-state"><div className="result-mark result-pending"><Icon name="activity" size={24} /></div><p className="eyebrow">Saved on this device</p><h1>Payment queued for later</h1><p className="state-description">We could not reach TransactX, so this payment intent is stored locally for a later submission. It is not completed yet.</p><div className="mini-payment"><Avatar name={recipient.name} /><span><strong>{recipient.name}</strong><small><Amount paise={amount} /></small></span></div><div className="confirm-actions"><Button onClick={onDone}>Back to overview</Button></div></div>; }
...
function UncertainPayment({ onRetry, onEdit }: { onRetry: () => void; onEdit: () => void }) { return <div className="focused-state centered-state"><div className="result-mark result-pending"><Icon name="activity" size={24} /></div><p className="eyebrow">Payment status</p><h1>We couldn't confirm the response</h1><p className="state-description">Your payment attempt timed out or lost connection. <strong>Money may have already moved downstream.</strong> We have preserved your original payment attempt ID so you can safely verify status without risk of a duplicate charge.</p><div className="confirm-actions"><Button variant="secondary" onClick={onEdit}>Edit details</Button><Button onClick={onRetry}>Re-check payment safely</Button></div></div>; }
function PaymentResult({ payment, token, onPayment, onView, onDone }: { payment: Payment; token: string; onPayment: (payment: Payment) => void; onView: () => void; onDone: () => void }) { const copy = paymentResultCopy(payment); const [refreshing, setRefreshing] = useState(false); const counterparty = payment.direction === "RECEIVED" ? payment.senderName : payment.receiverName; async function refresh() { setRefreshing(true); try { onPayment(await api.payment(payment.id, token)); } finally { setRefreshing(false); } } return <div className="focused-state centered-state"><div className={`result-mark result-${copy.tone}`}><Icon name={copy.tone === "success" ? "check" : copy.tone === "error" ? "close" : "activity"} size={24} /></div><p className="eyebrow">Payment update</p><h1>{copy.title}</h1><p className="state-description">{copy.description}</p><Amount paise={payment.amountPaise} prominent /><div className="result-recipient">{payment.direction === "RECEIVED" ? "From" : "To"} <strong>{counterparty}</strong><span>{payment.direction === "RECEIVED" ? payment.senderPaymentIdentifier : payment.receiverPaymentIdentifier}</span></div>{payment.note && <p className="result-note">"{payment.note}"</p>}<div className="confirm-actions">{payment.state !== "COMPLETED" && payment.state !== "FAILED" && payment.state !== "REVERSED" && <Button variant="secondary" onClick={() => void refresh()} disabled={refreshing}>{refreshing ? "Checking…" : "Check status"}</Button>}<Button variant="secondary" onClick={onDone}>Back to overview</Button><Button onClick={onView}>View transaction <Icon name="arrow" /></Button></div></div>; }
...
function DetailsView({ paymentID, token, onNavigate }: { paymentID: string; token: string; onNavigate: (view: View) => void }) { const [payment, setPayment] = useState<Payment | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); useEffect(() => { setLoading(true); api.payment(paymentID, token).then(setPayment).catch((caught) => setError(caught instanceof Error ? caught.message : "Transaction not found")).finally(() => setLoading(false)); }, [paymentID, token]); if (loading) return <DetailsSkeleton />; if (error || !payment) return <div className="center-state"><InlineError message={error || "Transaction not found"} /><Button variant="secondary" onClick={() => onNavigate("transactions")}>Back to transactions</Button></div>; const copy = paymentResultCopy(payment); const recipientLabel = payment.direction === "RECEIVED" ? "Sender" : "Recipient"; const counterparty = payment.direction === "RECEIVED" ? payment.senderName : payment.receiverName; const identifier = payment.direction === "RECEIVED" ? payment.senderPaymentIdentifier : payment.receiverPaymentIdentifier; return <div className="details-page"><button className="back-link" onClick={() => onNavigate("transactions")}><Icon name="arrow" size={15} />Back to transactions</button><div className="details-heading"><div><p className="eyebrow">Transaction</p><h1>Payment details</h1></div><StatusBadge state={payment.state} /></div><section className="details-hero"><div><span className="detail-label">Amount</span><Amount paise={payment.amountPaise} prominent /><p className="detail-state">{copy.description}</p></div><div className="details-recipient"><Avatar name={counterparty} /><span><small>{recipientLabel}</small><strong>{counterparty}</strong><span>{identifier}</span></span></div></section><section className="record-section"><p className="eyebrow">Record</p><div className="record-list"><Record label="Status"><StatusBadge state={payment.state} /></Record><Record label="Direction" value={payment.direction === "RECEIVED" ? "Received" : "Sent"} /><Record label="Created" value={formatDate(payment.createdAt)} /><Record label="Completed" value={payment.completedAt ? formatDate(payment.completedAt) : "Not yet confirmed"} />{payment.durationMs !== undefined && <Record label="Duration" value={`${payment.durationMs} ms`} />}{payment.sourceBankName && <Record label="Source bank" value={`${payment.sourceBankName}${payment.sourceBankCode ? ` (${payment.sourceBankCode})` : ""}`} />}{payment.destinationBankName && <Record label="Destination bank" value={`${payment.destinationBankName}${payment.destinationBankCode ? ` (${payment.destinationBankCode})` : ""}`} />}{payment.note && <Record label="Note" value={payment.note} />}<Record label="Origin" value={payment.origin} /><Record label="Reference"><div className="record-reference"><strong className="mono">{payment.id}</strong><CopyButton text={payment.id} /></div></Record></div></section>{payment.failureReason && <div className="detail-alert"><Icon name="alert" size={17} /><span>{payment.failureReason}</span></div>}</div>; }
function NetworkStatus({ online }: { online: boolean }) {
  if (online) return <div className="network-status network-online"><span className="status-dot" />ONLINE</div>;
  return <div className="network-status network-offline"><span className="status-dot" />OFFLINE<small>Payment intents are stored locally and will be replayed when reconnected.</small></div>;
}
function OfflineQueuePanel({ intents, onRetry }: { intents: OfflineIntent[]; onRetry: (clientRequestId: string) => Promise<void> }) { if (intents.length === 0) return null; return <section className="offline-queue-panel"><div className="section-heading"><div><p className="eyebrow">Offline sync</p><h2>Payment intents</h2></div><span className="toolbar-note">{intents.length} retained</span></div><div className="offline-intent-list">{intents.map((intent) => <div className="offline-intent-row" key={intent.clientRequestId}><div><strong>{intent.payload.recipient}</strong><small><Amount paise={intent.payload.amountPaise} /> · {formatDate(intent.createdAt)}</small></div><div className="offline-intent-status"><StatusBadge state={intent.replay?.resolution === "PENDING" ? "PENDING" : intent.state} />{intent.state === "FAILED" && <Button variant="secondary" onClick={() => void onRetry(intent.clientRequestId)}>Retry safely</Button>}</div></div>)}</div></section>; }
