...
import { Amount, Avatar, BrandMark, Button, EmptyState, Field, formatDate, Icon, InlineError, PageHeader, PaymentRow, paymentResultCopy, Skeleton, StatusBadge } from "./components";
...
function NetworkStatus({ online }: { online: boolean }) { return <div className={`network-status ${online ? "network-online" : "network-offline"}`}><span className="status-dot" />{online ? "ONLINE" : "OFFLINE"}<small>{online ? "Connection is available; replay still confirms reachability." : "Payment intents can be stored safely for later."}</small></div>; }
