...
func (data repositoryTestData) close(t *testing.T) {
	...
	_, _ = data.pool.Exec(context.Background(), `DELETE FROM payment_route_decisions WHERE payment_id IN (SELECT id FROM payments WHERE initiated_by_user_id = $1)`, data.userID)
